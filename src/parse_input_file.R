...
# assign misc variables variables
global_params = global_input[!(global_input[[2]] %in% c("hex_grid", "id_columns", "constraints")),]
...
temp = global_input[global_input[[2]]=="hex_grid", 4:ncol(global_input)]
temp = temp[temp!=""]
if(length(temp)>0){
  for(g in seq(length(temp))){
    ...
    if(any(grep(",", entry[2]))){
      items = parse_guess_custom(unlist(str_split(entry[2], ","))) # split and parse
      hex_load_params[[entry[1]]] <- items # add to hex grid filter list
    }else{hex_load_params[[entry[1]]] <- parse_guess_custom(entry[2])} # if no comma separating items add the parameter directly
  } # g loop
} # if length temp
## assign constraints variables
constr_global <<- list()
if(any(global_input[,2]=="constraints")){
  constr_global["fp_in"] = file.path(sub("/$", "", project_dir),"data/model_data",global_input[global_input[[2]]=="constraints",3])
...
for(n in seq(nrow(layer_input))){
  ...
  if(length(load_params_raw)>0){
    for(l in seq(length(load_params_raw))){
      ...
      }else{load_params[[entry[1]]] <- parse_guess_custom(entry[2])} # if no comma seperating items add the parameter directly
    } # l load params loop
  } # if length load > 0
  ### 
  ...
  if(length(score_params_raw)>0){
    for(s in seq(length(score_params_raw))){
      ...
      }else{score_params[[entry[1]]] <- parse_guess_custom(entry[2])} # if no comma seperating items add the parameter directly
    } # s score params loop
  } # if score length > 0
  ###########################################################
  ...
  lyr = list(
    ...
    fp_data_in = file.path(sub("/$", "", input_dir), layer_row$fp_in),
    ...
    weight = parse_guess_custom(layer_row$weight)
  )
  # add to model_layers
  model_layers[[n]] = lyr
} # n layer_input loop
...
dir.create(file.path(sub("/$", "", project_dir), "data/model_data"), showWarnings = FALSE)
